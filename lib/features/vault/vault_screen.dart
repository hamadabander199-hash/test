...
/// مش كل الصفوف مرة واحدة. شكل الخلية نفسه (VaultGridCell) موجود في
/// vault_screen_style.dart.
...
class _VaultGalleryContentState extends State<_VaultGalleryContent> {
  ...
  Future<void> _importFile() async {
    ...
    // بنبلّغ الـ VaultAuthGate إننا هنفتح واجهة خارجية متوقعة (file
    // picker) دلوقتي، عشان يتجاهل الـ pause/resume الناتجة عنها
    // ومايقفلش الشاشة تحتنا لحظة ما المستخدم لسه بيختار الملف. الـ
    // finally هنا ضروري عشان الفلاج ميفضلش عالق لو حصل أي exception.
    VaultAuthGate.beginExternalIntent();
    try {
      ...
      _debug('فشل فتح الـ file picker: $e\n$st');
      _toastError('تعذّر فتح شاشة اختيار الملفات');
      return;
    } finally {
      VaultAuthGate.endExternalIntent();
    }
    ...
    // فحص دفاعي إضافي: لو الشاشة اتقفلت لأي سبب تاني غير متوقع أثناء
    // فترة الـ picker (حتى مع وجود beginExternalIntent)، منكملش.
    if (!mounted) {
      _debug('الشاشة اتقفلت أثناء اختيار الملف - بنوقف هنا');
      return;
    }
    ...
    try {
      ...
      _debug(
          'نجحت عملية الاستيراد، id الملف الجديد = ${item.id}, نوعه = ${item.type}');
